...
type AppData struct {
	PersonList        *personll.LinkedList
	BstUserName       *btree.BST
	BstID             *btree.BST
	ApptArray         []time.Time
					...
					if idNode == nil { // ID is not in BST
						...
						if usernameNode == nil { // username is not in BST
							...
							if err != nil {
								...
								Wg.Add(2)
								filehandler.WritePersonCSVFile(Wg, a.PersonList)
								filehandler.WriteApptCSVFile(Wg, a.ApptArray)
								Wg.Wait()
...
func (a *AppData) getPerson(w http.ResponseWriter, req *http.Request) person.Person {
...
func (a *AppData) updatePerson(p person.Person, w http.ResponseWriter, req *http.Request) {
	// get current session cookie
	myCookie, err := req.Cookie("VaccineAppt")
