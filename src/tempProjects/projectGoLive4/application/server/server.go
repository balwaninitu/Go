				...
				if verr != nil { // error
					...
					if !serr { // error during sanitization
						...
						if _, ok := mapUsers[snInput.Username]; ok {
							...
							fmt.Println("Username:", newUserdb.Username)
							fmt.Println("Password:", newUserdb.Password)
			...
			if !err {
				...
				if findInMap(mapSessions, lInput.Username) {
					...
					if lInput.Username == config.AdminName {
						fmt.Println("Checking password", config.AdminName, config.AdminPw, lInput.Password)
						...
						if _, ok := mapUsers[lInput.Username]; !ok { // username is not registered
							...
							if !ok {
								...
								if err != nil {
									...
									//fmt.Println("User details :", details)
									...
									if err != nil {
										...
										//fmt.Println("Map value:", mapUsers)
										isbuyer = mapUsers[lInput.Username].IsBuyer
										//fmt.Println("From map :", isbuyer)
										if isbuyer {
...
func LogoutHandler(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		fmt.Println("Deleting cookie and clearing session..")
		...
		// can add timer , then redirec to home
		//http.Redirect(w, req, "/", http.StatusSeeOther)
		//return
	}
