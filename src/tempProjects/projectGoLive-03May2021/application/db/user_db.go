...
func GetRecords(db *sql.DB) ([]UserDetails, bool) {
	...
	for results.Next() {
		...
		err = results.Scan(&uinfo.Username, &uinfo.Password, &uinfo.Isbuyer, &uinfo.Location)
		if err != nil {
...
func GetARecord(db *sql.DB, uname string) (UserDetails, bool) {
	...
	for results.Next() {
		// map this type to the record in the table
		err = results.Scan(&ud.Username, &ud.Password, &ud.Isbuyer, &ud.Location)
		if err != nil {
...
func InsertRecord(db *sql.DB, ud UserDetails) bool {
	query := fmt.Sprintf("INSERT INTO `Userdetails`(Username, Password, Isbuyer, Location) VALUES ('%s','%s',%t,'%s')", ud.Username, ud.Password, ud.Isbuyer, ud.Location)
	//query := fmt.Sprintf("INSERT INTO `Userdetails`(Username, Password, Isbuyer, Location) VALUES (?,?,?,?)", ud.Username, ud.Password, ud.Isbuyer, ud.Location)
	//_, err := db.Exec("INSERT INTO Userdetails (Username, Password, Isbuyer, Location) VALUES (?,?,?,?)", ud.Username, ud.Password, ud.Isbuyer, ud.Location)
	_, err := db.Exec(query)
	if err != nil {
...
func EditRecord(db *sql.DB, uname string, ud UserDetails) bool {
	query := fmt.Sprintf("UPDATE `Userdetails` SET Username='%s', Password='%s', Isbuyer=%t, Location='%s' WHERE Username='%s'", ud.Username, ud.Password, ud.Isbuyer, ud.Location, uname)
