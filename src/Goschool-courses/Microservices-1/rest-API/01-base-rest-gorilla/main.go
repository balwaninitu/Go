...
// const baseURL = "http://api/v1/courses"
...
//used for storing courses on REST API
...
func validKey(r *http.Request) bool {
	...
	if key, ok := v["key"]; ok {
		...
		return false //key is not provided
	}
}
	...
	if r.Header.Get("Content-type") == "application/json" {
		if r.Method == "POST" {
			...
			if err == nil {
				...
				//check if course exist; add only if course does not exist
				if _, ok := courses[params["courseid"]]; !ok {
				...
				w.WriteHeader(
					http.StatusUnprocessableEntity)
				w.Write([]byte("422 - Please provide course information " +
					"in JSON format"))
			}
			//put is for updating
			//existing course
