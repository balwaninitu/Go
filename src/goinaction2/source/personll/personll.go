...
func (p *LinkedList) Get(index int) (user.Person, error) {
	emptyItem := user.Person{}
	if p.Head == nil {
...
func (p *LinkedList) AddNode(name user.Person, wglocal *sync.WaitGroup) error {
	defer wglocal.Done()
...
func (p *LinkedList) AddAtPos(index int, name user.Person) error {
	newNode := &Node{
		Item: name,
		Next: nil,
	}
...
func (p *LinkedList) Remove(index int) (user.Person, error) {
	var item user.Person
	emptyItem := user.Person{}
...
func (p *LinkedList) SearchUserName(username string) (user.Person, int, error) {
	emptyItem := user.Person{}
	index := 1
	if p.Head == nil {
...
func (p *LinkedList) WriteAtIndex(index int, thisPerson user.Person) error {
...
func (p *LinkedList) WritePersonData(currentPerson user.Person, currentPersonIndex int) error {
