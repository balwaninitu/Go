...
func init() {
	...
	// Create loggers for logging Trace, Info, Warning and Error messages
	fh.Trace = log.New(ioutil.Discard,
		"TRACE: ",
		log.Ldate|log.Ltime|log.Lshortfile)

	fh.Info = log.New(os.Stdout,
		"INFO: ",
		log.Ldate|log.Ltime|log.Lshortfile)

	fh.Warning = log.New(os.Stdout,
		"WARNING: ",
		log.Ldate|log.Ltime|log.Lshortfile)

	fh.Error = log.New(io.MultiWriter(fhfile, os.Stderr),
		"ERROR: ",
		log.Ldate|log.Ltime|log.Lshortfile)
	...
	if err != nil {
		fh.Warning.Printf("Unable to open Appointment Available CSV file: %v\n", err)
	}

	// Check if CSV Appointment file can be read
	_, err = csv.NewReader(csvapptfile).ReadAll()
	if err != nil {
		fh.Warning.Printf("Unable to read Appointment Available CSV file: %v\n", err)
	}
	csvapptfile.Close()
	...
	if err != nil {
		fh.Warning.Printf("Unable to remove temporary Person CSV file: %v\n", err)
	}
...
func main() {
	...
	appDS.ApptArray = fh.ReadApptCSVFile(&wg)

	// Read all person data from CSV file
	appDS.PersonList, appDS.BstUserName, appDS.BstID = fh.ReadpersonCSVFile(&wg)

	// Read Admin Login and Password information
	appDS.AdminLogin.AdminName, appDS.AdminLogin.AdminPassword = fh.ReadAdminCSVFile()
